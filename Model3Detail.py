...
class MODEL3DETAIL(object):
    ...
    def _fill_na(self, pred_day: str):
        ...
        for model, df in self.rec_input.items():
            ...
            df['avail_capa'].fillna(self.capa_re[(pred_mon, self.model_map[model])])
    ...
    def _load_init_data(self):
        ...
        self.capa_re = {(month, model): capa for month, model, capa in zip(curr_capa['date'],
                                                                           curr_capa['model'],
                                                                           curr_capa['capa'])}
