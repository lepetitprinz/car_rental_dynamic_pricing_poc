...
def data_preprocessing(status_update_day: str):
    data_prep = DataPrep()
    # History dataset
    data_prep.prep_res_hx()
    # Recent  dataset
    data_prep.prep_res_recent(status_update_day=status_update_day)
...
def main():
    ...
    start_date = '2020/12/07'           # 할인율 산출 기간: 시작
    end_date = '2021/02/28'             # 할인율 산출 기간: 끝
    apply_day = '2020/12/07'            # Discount apply day (할인율 적용일)
    ...
    disc_confirm_last_week = '201127'        # Discount dataset on previous week (이전 확정 할인율)
    ...
    # Data Preprocessing
    # data_preprocessing(status_update_day=res_status_ud_day)
